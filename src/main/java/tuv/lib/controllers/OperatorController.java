...
public class OperatorController implements Initializable {
	...
	private void makeRent(ActionEvent event) throws SQLException {
		...
		/*
		 * SELECT books.BOOK_ID,rents.RENT_STATUS FROM ( SELECT books.BOOK_ID from books
		 * join books_info on books_info.BOOK_INFO_ID = books.BOOK_INFO_ID WHERE
		 * books_info.BOOK_INFO_NAME = 'The Notebook' ) books left JOIN rents on
		 * rents.BOOK_ID = books.BOOK_ID where rents.RENT_STATUS is null or
		 * rents.RENT_STATUS = 1 LIMIT 1;
		 */
